...
import com.tecgvg.gyantatv.service.DistrictService;
import com.tecgvg.gyantatv.service.criteria.DistrictCriteria;
...
class DistrictResourceIT {
    ...
    @Test
    @Transactional
    void getAllDistrictsByClientDetailsIsEqualToSomething() throws Exception {
        // Initialize the database
        districtRepository.saveAndFlush(district);
        ...
        em.persist(clientDetails);
        em.flush();
        district.setClientDetails(clientDetails);
        clientDetails.setDistrict(district);
        districtRepository.saveAndFlush(district);
        Long clientDetailsId = clientDetails.getId();

        // Get all the districtList where clientDetails equals to clientDetailsId
        defaultDistrictShouldBeFound("clientDetailsId.equals=" + clientDetailsId);

        // Get all the districtList where clientDetails equals to (clientDetailsId + 1)
        defaultDistrictShouldNotBeFound("clientDetailsId.equals=" + (clientDetailsId + 1));
    }
